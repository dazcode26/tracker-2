  ...
  const renderMonthView = () => {
    ...
    return (
      ...
      >
        ...
        {/* Weeks Grid */}
        <div
          className="grid flex-1 divide-y divide-[#27272a] min-h-0"
          style={{ gridTemplateRows: `repeat(${weeks.length}, minmax(0, 1fr))` }}
        >
          {weeks.map((week, wIdx) => (
            <div key={wIdx} className={`grid ${showWeekends ? 'grid-cols-7' : 'grid-cols-5'} divide-x divide-[#27272a] min-h-0`}>
              {week.map((date) => {
                ...
                return (
                  <div
                    ...
                    className={`group relative p-1.5 sm:p-2 flex flex-col transition-colors min-h-0 overflow-hidden ${
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-1 select-none shrink-0">
                      ...
                      <button
                        ...
                        className="opacity-0 group-hover:opacity-100 p-0.5 sm:p-1 rounded hover:bg-[#27272a] text-[#a1a1aa] hover:text-white transition-all cursor-pointer"
  ...
  const renderDayOrWeekView = () => {
    ...
    return (
      <div className="flex flex-col flex-1 bg-[#121215] border border-[#27272a] rounded-xl overflow-hidden shadow-xl min-h-0 md:h-full">
        ...
        <div className="flex border-b border-[#27272a] bg-[#141417] min-h-[38px] z-10 shrink-0">
        ...
        >
          ...
          >
            ...
            <div
              className="w-16 sm:w-20 border-r border-[#27272a] bg-[#141418]/60 shrink-0 select-none"
              style={{ height: `${24 * hourHeight}px` }}
            >
              {hours.map((hr) => (
                <div
            ...
            <div
              className={`grid flex-1 ${isWeek ? (showWeekends ? 'grid-cols-7' : 'grid-cols-5') : 'grid-cols-1'} divide-x divide-[#27272a] relative`}
              style={{ height: `${24 * hourHeight}px` }}
            >
            {dayColumns.map((d) => {
              const dKey = formatDateKey(d);
